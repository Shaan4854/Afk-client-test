...
async function testConnectionResilience() {
  ...
  // Informational, not a hard requirement: connection.js always resolves a
  // concrete version before calling createBot, which means mineflayer's own
  // internal auto-version-ping path (the actual source of the double-error
  // crash this listener guards against) never runs in the first place. The
  // listener stays as cheap insurance for any other late-error scenario;
  // it just may not have anything to catch in this particular test.
  console.log(`[INFO] late post-disconnect error event observed: ${sawLateErrorAbsorbed}`);
}
